...
2. 独行公式：将公式插入到新的一行内，并且居中，(Typora 不好使 )符号：`$$公式内容$$`，如：$$xyz$$

## 上标、下标与组合

1. 上标符号，符号：`$正常内容^上标内容$`，如：$x^4$
2. 下标符号，符号：`$正常内容_下标内容$`，如：$x_1$
3. 上下标组合符号，符号：`${正常内容}_{下标内容}{正常内容}^{上标内容}$`，如：${16}_{8} O{2+}_{2}$ ，$ {正常内容}_{下标内容}{正常内容}^{上标内容}$
